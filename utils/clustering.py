...
from utils.utility import select_dataset,mkdir,mean_of_top_n_elements
...
def dataclustering(n_clusters=10):
    from sklearn_extra.cluster import KMedoids
    means = []

    rc = pd.read_table(f'./chembl31/chembl31/chemblx31-Ki/curated/curation_summary.tsv',
                    index_col=0, header=0)
    actives = select_dataset(rc,
                            subset='num_of_compounds(after curation)',
                            col_name='actives',
                            threshold=1000)

    for active in actives:
        path = f'./chembl31/chembl31/chemblx31-Ki/curated/{active}_curated.tsv'
        tid = pd.read_table(path, header=0,
                        index_col=0)
        mols = [Chem.MolFromSmiles(mol) for mol in tid['washed_kekule_smiles']]
        ecfps = np.mean([[bit for bit in AllChem.GetMorganFingerprintAsBitVect(mol,2,8192)]
                for mol in mols],axis=0).tolist()
        means.append(ecfps)
        print(f"{active} done.")

    means_df = pd.DataFrame(means,index=actives)
    means_ar = means_df.to_numpy()

    meds = KMedoids(n_clusters=n_clusters,random_state=0).fit(means_ar)
    centers = meds.cluster_centers_
    indices = [index for index, element in enumerate(means) if element in centers.tolist()]

    means_df

    indexs = means_df.index[indices].to_list()

    print(f"-----Selected datasets by k-Medoids (Num-of-center : {n_clusters})-----")
...
class neighbors:
    ...
    def calcsims(self,bits_array_,split=10000):
        ...
        for iter, bits_array in enumerate(bits_array_list):
            ...
            # for i,ind_ in enumerate(tqdm(inds)):
            #     sim_ = []
            #     for ind in ind_:
            #         bits_list_tr = self.bits_array_train[ind]
            #         bits_tr = "".join(str(bit) for bit in bits_list_tr)
            #         bits = "".join(str(bit) for bit in bits_array[i])
            #         fp_tr = CreateFromBitString(bits_tr)
            #         fp = CreateFromBitString(bits)
            #         sim = TanimotoSimilarity(fp_tr,fp)
            #         sim_.append(sim)
            #     sims.append(np.mean(np.array(sim_)))
