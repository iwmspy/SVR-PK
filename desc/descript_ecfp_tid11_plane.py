...
def ecfp(mol_list, fr=None):
    ecfp_list = []
    ecfp_bitinfo = []
    if fr != None:
        ecfp_col = [f"fr{fr}_ecfp_bit_{i}" for i in range(1024*8)]
        ecfp_col[0:0] = [f'Fragment{fr}_ECFP',f'Fragment{fr}_ECFP_mol']
    else:
        ecfp_col = [f"ecfp_bit_{i}" for i in range(1024*8)]
        ecfp_col[0:0] = [f'Product_ECFP',f'Product_ECFP_mol']
    for list in mol_list:
        ecfp_list_tmp = []
        ecfp_bitinfo_tmp = {}
        ecfp = AllChem.GetMorganFingerprintAsBitVect(list[1], 2, 1024*8, bitInfo=ecfp_bitinfo_tmp)
