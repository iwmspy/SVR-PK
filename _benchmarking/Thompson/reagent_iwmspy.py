"""
Comparison of DisallowTracker implementations:

This docstring highlights the differences between the two versions of the `DisallowTracker` class.

### Key Differences:

1. **Random Number Generator (`rng_tg`) Initialization**:
   - **Old Version**: Uses `random.shuffle` for shuffling the selection order in the `sample` method.
   - **New Version**: Introduces `rng_tg = np.random.default_rng(seed=0)` to create a reusable random number generator instance. This replaces `random.shuffle` with `rng_tg.permutation` for deterministic and reproducible shuffling.

   **Reason**: The new version uses `np.random.default_rng` for better reproducibility and to avoid modifying the global random state.
"""
import numpy as np
