...
1. **Random Number Generator (`rng_tr`) Initialization**:
   - **Old Version**: Uses `random.shuffle` for shuffling the selection order in the `sample` method.
   - **New Version**: Introduces `rng_tr = np.random.default_rng(seed=0)` to create a reusable random number generator instance. This replaces `random.shuffle` with `rng_tr.permutation` for deterministic and reproducible shuffling.
